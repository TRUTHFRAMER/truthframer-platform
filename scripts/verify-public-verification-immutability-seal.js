...
function fail(msg) {
  console.error(`TRUTHFRAMER_PUBLIC_VERIFICATION_IMMUTABILITY_SEAL_FAIL=${msg}`);
  process.exit(1);
}
...
if (seal.status !== "PUBLIC_VERIFICATION_IMMUTABILITY_CERTIFIED_SEALED") fail("SEAL_STATUS_MISMATCH");
if (seal.certificate_sha256 !== sha256File(certPath)) fail("CERTIFICATE_HASH_MISMATCH");

console.log("TRUTHFRAMER_PUBLIC_VERIFICATION_IMMUTABILITY_SEAL_PASS=true");
console.log(`SEAL_VERSION=${VERSION}`);
console.log(`STATUS=${seal.status}`);
console.log(`SEALED_CERTIFICATE_SHA256=${seal.certificate_sha256}`);
